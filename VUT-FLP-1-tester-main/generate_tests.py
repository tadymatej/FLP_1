...
from config import N_SAMPLES_TREE, NEW_DATA_COUNT
...
def generate_test(idx, feature_size=FEATURE_SIZE, n_samples_tree=N_SAMPLES_TREE, new_data_count=NEW_DATA_COUNT):
    ds = create_dataset(n_samples=n_samples_tree, feature_size=feature_size, class_list=create_class_list(n_classes=10))
    ...
    new_data = create_dataset(n_samples=new_data_count, feature_size=feature_size)
    print_new_data_into_file(new_data, idx)

    classifications = classify_new_data(t, new_data)
