----------------------------------------------------------------------------------------
--- Project name: FLP - project1 - decision tree ---------------------------------------
--- Author: Matěj Žalmánek (xzalma00) --------------------------------------------------
--- Date: 26.3. 2024 -------------------------------------------------------------------
----------------------------------------------------------------------------------------

--- Implemented extensions: 
--    accepting scientific notation of numbers, 
--    more benevolent for multiple spaces when parsing input data (but not totally benevolent)
...
-- | updates unique count. If it will find certain key in input array, it will increase number of occurrences in this array by one
-- Otherwise, it will append to this array a new item with initial value of number of occurrences = 1
updateUniqueCount :: (Eq a) => 
    [(a, Int)] -> -- ^ Input array in which we are trying to increment number of occurrences
    a -> -- ^ Key which we are trying to find in the input array
    [(a, Int)]
...
-- | returns array of unique items and its number of occurrences. For example for array: "aahojjj"
-- result will be: [('a', 2), ('h', 1), ('o', 1), ('j', 3)]
-- It allows to use this function to get unique items from complex structures and map property of this complex structure
-- to compute unique occurrences. For example for array: [[1, 2, 3, "aa"], [1,2,2, "aa"], [1, 2, 2, "bb"]], when we provide
-- function that extracts last item (String), this function will return:
-- [("aa", 2), ("bb", 1)]
uniqueCount :: (Eq b) => 
    (a -> b) -> -- ^ Function to extract property, which is used as a comparing key
    [a] -> --  ^ Array from which we are computing unique items
    [(b, Int)]
uniqueCount _f [] = []
...
-- | calculates the gini index for decision tree learning by the following formula:
-- gini = 1 - sum(P(Class_i)^2), where P(Class_i) is probability of class i
giniScore :: 
    [TrainingData] -> -- ^ Training data, from which we are calculating gini index
    Double 
giniScore classes =
...
-- | splits TrainingData array into two arrays. Splitting is performed by attributeIndex and attributeThreshold.
-- TrainingData with attribute value <= attributeThreshold (at certain attributeIndex) goes to the left TrainingData array,
-- otherwise it goes to the right TrainingData array
splitData :: 
    Int ->  -- ^ AttributeIndex by which we are splitting
    Double -> -- ^ Attribute threshold by which we are splitting
    [TrainingData] -> -- ^ TrainingData which we are splitting
...
-- | finds best split props (attributeIndex, attributeThreshold) for given TrainingData array
findBestSplit :: 
    [TrainingData] -> -- ^ TrainingData array which we are trying to split to two arrays with best split
    LearnTrainingTreeProps
...
-- | Gets midpoints arrays from TrainingData array. left midpoint array has all values for certain attributeIndex at TrainingData. Second array is reversed first array.
-- This function skips same values for the same attribute. It is required to sort the input TrainingData by AttributeIndex (attributeIndex which is used in this function) before calling this function.
getMidpointArrays :: 
    [TrainingData] -> -- ^ TrainingData from which we want to create midpoint arrays
    Int -> -- ^ AttributeIndex from which we want to create midpoint arrays
    ([Double], [Double])
getMidpointArrays trainingData attributeIndex = 
    ...
    where 
        -- | Helper function for recursion and appending to the midpoint array
        getMidpointArrayFn :: 
            [TrainingData] -> -- ^ TrainingData from which we want to create midpoint arrays
            Int -> -- ^ AttributeIndex from which we want to create midpoint arrays
            (Bool, Double) -> -- ^ (Is it first midpoint value?, previous attribute value (previous iteration), for skip)
            [Double]
...
-- | Calculates midpoints by the following formula: (left_i, right_i) = ((left_i + right_i) / 2)
calcMidpoints :: 
    ([Double], [Double]) -> -- ^ Values to compute midpoints. Size of these two arrays should be equal.
                                -- result with calculated midpoints will be also with this array size
    [Double]
...
-- | Computes best split for certain attribute. If its better then given best split, it will return new best split, 
-- otherwise it will return the given best split
findBestSplitProcessAttributes :: 
    [TrainingData] -> -- ^ TrainingData from which we are computing the best split
    Int -> -- ^ attributeIndex to process
    Int -> -- ^ number of attributes
    LearnTrainingTreeProps -> -- Actual best split props (bestGini, attributeIndex, attributeThreshold)
    LearnTrainingTreeProps
findBestSplitProcessAttributes trainingData processingAI attributesCount learnProps@(LearnTrainingTreeProps prevBestGini _prevAI _prevAT) =
        ...
        where 
            ...
            recursiveCall :: 
                LearnTrainingTreeProps -> -- ^ LearnTrainingTreeProps with BEST split informations
                LearnTrainingTreeProps
            ...
            -- | Selects best split props from two LearnTrainingTreeProps
            selectBestProps :: 
                LearnTrainingTreeProps -> -- ^ newly calculated (gini, attributeIndex, attributeThreshold)
                LearnTrainingTreeProps 
...
-- | finds best split (threshold for certain attributeIndex) from midpoint values.
findBestSplitProcessMidpoints :: 
    [Double] -> -- ^ Array with midpoints
    [TrainingData] -> -- ^ TrainingData from which we are learning
    Int -> -- ^ Attribute index at which we have computed midpoints
    LearnTrainingTreeProps -> -- ^ Actual best split attributes (bestGini, attributeIndex, attributeThreshold)
    LearnTrainingTreeProps
...
findBestSplitProcessMidpoints (threshold : restMidpoints) trainingData processingAI learnProps =
    ...
    where 
        -- returns best training props from two given props (bestGini, bestAttributeIndex, bestAttributeThreshold)
        getBestGini :: 
            LearnTrainingTreeProps -> -- ^ first LearnTrainingTreeProps to compare
            LearnTrainingTreeProps -> -- ^ second LearnTrainingTreeProps to compare
            LearnTrainingTreeProps 
        ...
        -- | helper function for recursion
        callRecursive :: 
            LearnTrainingTreeProps -> -- ^ Actual best split attributes
            LearnTrainingTreeProps
...
-- | helper function to provide comparing function for TrainingData sorting by a certain attributeIndex
trainingDataSortFnc :: 
    Int -> -- ^ attributeIndex by which we are comparing
    TrainingData -> -- ^ One of the comparing TrainingData
    TrainingData -> -- ^ Second of the comparing TrainingData
    Ordering
...
-- | Builds the DecisionTree (learns the best split attributeIndex and attributeThreshold)
buildTree :: 
    [TrainingData] -> -- Training data with which we learn to build a decision tree
    DecisionTree Int Double String
buildTree [] = EmptyTree
buildTree (trainingData@(TrainingData _doubles className): restTrainingData) = do
    ...
    where
        -- | Helper function to check if we should continue in recursion, or we can append Leaf node to our tree
        buildTreeHelperFn :: 
            [TrainingData] -> -- ^ Training data with which we learn to build a decision tree
            LearnTrainingTreeProps -> -- ^ learned best split attributes (attributeIndex, attributeThreshold)
            DecisionTree Int Double String 
